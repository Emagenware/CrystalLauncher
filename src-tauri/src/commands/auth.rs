...
pub async fn refresh_login(refresh_token: String) -> Result<MinecraftAccount, String> {
    ...
    if !msa_res.status().is_success() {
        let body = msa_res.text().await.unwrap_or_default();
...
async fn perform_handshake(code: &str) -> Result<MinecraftAccount, String> {
    ...
    if !msa_res.status().is_success() {
        return Err(format!(
            "MSA Token Failed: {}",
            msa_res.text().await.unwrap_or_default()
        ));
    }
...
async fn perform_minecraft_login(
    client: &Client,
    msa_token: &str,
    ...
    if !xsts_res.status().is_success() {
        return Err(format!(
            "XSTS Auth Failed: {}",
            xsts_res.text().await.unwrap_or_default()
        ));
    }
